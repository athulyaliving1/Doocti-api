...
const mobileNumberSearch = (req, res) => {
  ...
  if (mobilenumber === "") {
    ...
    const queryAALDB = `SELECT patients.first_name,
    ...
    patients.email,
    patients.gender,
    master_enquiry_source.source_name,
    patients.current_area,
    patients.current_city,
    patients.current_state,
    patients.current_pin_code,
    patients.current_address,
    leads.followup_date,
    leads.comments,
    patients.relationship_with_patient,
    leads.enquirer_reference,
    leads.expected_accomodation_type,
    leads.proceed_to_assessment,
    master_branches.branch_name
    
FROM patients
JOIN leads
 ON patients.id = leads.patient_id
JOIN master_enquiry_source
 ON leads.enquiry_source_branch= master_enquiry_source.source_name
JOIN master_branches
 ON leads.branch_id = master_branches.id
 
WHERE patients.mobile_number = ?`;
